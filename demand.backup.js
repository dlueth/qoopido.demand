...
(function(global, document, configuration, JSON, arrayPrototype, objectPrototype, setTimeout, undefined) {
	...
	var /** shortcuts */
		...
		/** regular expressions */
			...
			regexMatchBaseUrl,
		/** classes */
			Uuid, Pledge, Queue, QueueHandler, Pattern, Xhr, Module, Loader, Failure,
		/** instances */
		 ...
		 */
			...
			function resolveDependency(dependency, context) {
				...
				if(isTypeOf(dependency, STRING_STRING)) {
					...
					return (registry[path] || (registry[path] = new Loader(dependency, context))).pledge;
				} else {
	 ...
	 */
		 ...
		 */
			Module = (function() {
				function Module(path, context) {
					...
					self.path     = resolvePath(path, context);
		 ...
		 * Loader
		 */
			Loader = (function(Module, settings) {
				...
				function Loader(path, context) {
					var self     = Module.call(this, path, context),
						deferred = Pledge.defer(),
					...
					demand(MODULE_PREFIX_HANDLER + self.handler)
						.then(
							function(handler) {
								...
								self.mock    = self.mock ? mocks[self.path] : NULL;
